...
include CLICKHEAT_ROOT.'data.php';
...
if (is_resource($f))
{
	...
	if ($logMe === true)
	{
                ...
                $ch_date = date('Y-m-d');
                $ch_content = ((int) $_GET['x']).'|'.((int) $_GET['y']).'|'.((int) $_GET['w']).'|'.$browser.'|'.((int) $_GET['c'])."\n";
                $time = time();
                $sql = "INSERT clickheat_log (ch_site,ch_date,ch_content,add_time) VALUE ('{$final}','{$ch_date}','{$ch_content}','{$time}')";
                mysql_query($sql,$con);
                mysql_close($con);
