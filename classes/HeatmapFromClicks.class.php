...
class HeatmapFromClicks extends Heatmap
{
	 ...
	 * @param string $file
	 */
	function addFile($file,$group,$date)
	{
		if (file_exists($file))
		{
			//$this->files[] = $file;
	...
	**/
	function drawPixels($image)
	{
		...
		for ($file = 0; $file < count($this->query); $file++)
		{
                        $buffer = $this->getClickHeatData($this->query[$file]['group'], $this->query[$file]['date']);
...
//			fclose($f);
	...
	**/
	 ...
	 * 存数据库读取数据
	**/
	function getClickHeatData($group,$date)
	{
                $con = mysql_connect(DB_HOST, DB_USER, DB_PASS);
                $db_selected = mysql_select_db(DB_NAME,$con);

                $sql = "select ch_content from clickheat_log where ch_site = '{$group}' and ch_date = '{$date}'";
